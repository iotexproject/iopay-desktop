...
import { Badge, Button, Card, Col, Dropdown, notification, Row, Table, Tooltip } from 'antd';
...
import { IAccountSectionProps, IAccountSectionState } from '../../../../interfaces/account-section.interface';
import { ITokenInfo, ITokenInfoDict } from '../../../../interfaces/wallet.interface';
import { xconf, XConfKeys } from '../../../../models/xconf.enum';
import { getAntenna } from '../../../../utils/get-antenna';

export class AccountSectionComponent extends React.Component<IAccountSectionProps, IAccountSectionState> {
  public state: IAccountSectionState = {
  ...
  public componentDidUpdate(): void {
    ...
    const accountCheckID = `${account && (account as Account)['address']}:${network && network.url}`;
  ...
  public pollAccount = async () => {
    ...
    if (account) {
      ...
      if (tokens.length > 0) {
        ...
        if (tokenAddress) {
          const claimableAmount = await this.claimableAmount(tokenAddress, account.address);
  ...
  public getTokensInfo = async () => {
    ...
    let tokenAddresses = xconf.getConf<Array<string>>(`${XConfKeys.TOKENS_ADDRS}_${accountCheckID}`, []);

    tokenAddresses = [...defaultNetworkTokens, ...tokenAddresses];
    const tokenInfos = await Promise.all(tokenAddresses.map((addr) => Token.getToken(addr).getInfo(account.address)));
  ...
  public emptyWallet = (): JSX.Element => {
    return (
      <Card bodyStyle={{ padding: 0 }} style={{ borderRadius: 5, height: '90%' }}>
        <img id="globe" className="blur-image" style={{ maxWidth: '100%', maxHeight: 400 }} alt="globe" src={assetURL('unlock-wallet.png')} />
  ...
  public onAddCustomToken = async (tokenAddress: string) => {
    ...
    try {
      const tokenInfo = await Token.getToken(tokenAddress).getInfo(account.address);
    ...
    xconf.setConf(`${XConfKeys.TOKENS_ADDRS}_${this.state.accountCheckID}`, Object.keys(tokenInfos));
    this.setState({
      customTokensFormVisible: false,
  ...
  public renderCustomTokenForm(): JSX.Element {
    return <AddCustomTokensFormModal visible={this.state.customTokensFormVisible} onOK={this.onAddCustomToken} onCancel={this.onClosedCustomTokenFormModal} />;
  }

  public claimVitaAs = async (tokenAddress: string, authMessage: IAuthorizedMessage) => {
    const { history, account } = this.props;
    if (!account) {
      return;
    }
    try {
      const txHash = await Token.getToken(tokenAddress).claimAs(authMessage, account);
      // @ts-ignore
      window.console.log(`Claimed VITA to ${authMessage.address} at action hash: ${txHash}`);
  ...
  public claimableAmount = async (tokenAddress: string, address: string): Promise<BigNumber> => Token.getToken(tokenAddress).claimableAmount(address);
    ...
    return (
      <AuthorizedMessageFormModal
        ...
        onOK={async (authMessage: IAuthorizedMessage) => {
          try {
            const gasEstimation = await Token.getToken(token.tokenAddress).estimateClaimAsGas(authMessage, account);
            this.setState({
              authorizedMessageFormVisible: false,
    ...
    return (
      <BidFormModal
        ...
        onOK={async (amount: string) => {
          try {
            const gasEstimation = await Token.getBiddingToken(bidContractAddress).estimateBidGas(account, amount);
            this.setState({
              bidFormModalVisible: false,
  ...
  public renderGenerateAuthorizedMessageFormModal(token: ITokenInfo): JSX.Element {
    return (
  ...
  public placeBid = async (amount: string) => {
    ...
    try {
      const txHash = await Token.getBiddingToken(bidContractAddress).bid(account, amount);
      // @ts-ignore
  ...
  private readonly onClaimClickHandle = (token: ITokenInfo) => async () => {
    ...
    try {
      const gasEstimation = await Token.getToken(token.tokenAddress).estimateClaimGas(account);
      this.setState({
        claimConfirmationVisible: true,
        claimTokenAddress: token.tokenAddress,
        authMessage: null,
  ...
  private renderClaimButton(token: ITokenInfo): JSX.Element {
    ...
    return (
      <Badge count={this.state.claimableAmount}>
        <Dropdown.Button overlay={claimMenu} className="claimButton" trigger={['click', 'hover']}>
          {
            // @ts-ignore
            <a
              type="primary"
  ...
  private renderSymbol(symbol: string, href: string, linkName: string): JSX.Element {
    return (
      <>
        <Row>{symbol}</Row>
        <Row>
          <a href={href} style={{ marginRight: 4, cursor: 'pointer' }} target="_blank noopener noreferer" onClick={onElectronClick(href)}>
            {t(linkName)}
          </a>
        </Row>
      </>
    ...
    return [
      {
        ...
        key: 'symbol',
        render: (text: string, token: ITokenInfo): JSX.Element | string | null => {
          if (token.symbol === 'IOTX') {
            return this.renderSymbol(token.symbol, VOTING_URL, 'wallet.vote.title');
          }
      ...
      },
      {
        ...
        key: 'balanceString',
        render: (text: string): JSX.Element | null => <span>{this.state.showBalance ? text : '******'}</span>,
      },
      {
        title: '',
  ...
  public renderBalance(): JSX.Element | null {
    ...
    if (accountMeta) {
      dataSource.unshift({
        symbol: 'IOTX',
        balanceString: new BigNumber(fromRau(accountMeta.balance, '')).toString(10),
        tokenAddress: '',
    ...
    const displayBalanceText = showBalance ? t('account.balance.hide') : t('account.balance.show');
    return (
      <SpinPreloader spinning={spinning}>
        ...
        <Table dataSource={dataSource} columns={columns} pagination={false} showHeader={false} rowKey={(record) => record.symbol} bodyStyle={{ overflowX: 'auto' }} />
      </SpinPreloader>
    );
  }
  ...
  public renderActionBar(): JSX.Element | null {
    ...
    return (
      <Row type="flex" justify="space-between" align="middle" style={{ padding: '10px 20px' }}>
        <Col>
          <div>
            <strong>{t('account.address')}</strong>
          </div>
          <div>
            <small>{account.address}</small> <CopyButtonClipboardComponent text={account.address} size="small" icon="copy" />
          </div>
        </Col>
        <Col>
          {isElectron() ? (
            <Link to="" target="_blank noopener noreferer" onClick={onElectronClick(`${(network !== undefined && network.url) || 'https://iotexscan.io/'}address/${account.address}`)}>
              {t('account.actionHistory')}
            </Link>
          ) : (
            <a target="_blank" rel="noreferrer" href={`${(network !== undefined && network.url) || 'https://iotexscan.io/'}address/${account.address}`}>
              {t('account.actionHistory')}
            </a>
          )}
        </Col>
  ...
  public renderWallet = (): JSX.Element | null => {
    ...
    return (
      <Card bodyStyle={{ padding: 0, wordBreak: 'break-word' }} style={{ borderRadius: 5 }}>
        <div
          style={{
        ...
        >
          <Row type="flex" justify="space-between" align="middle" style={{ paddingBottom: 5 }}>
            <Col style={{ cursor: 'pointer', fontSize: 13 }} onClick={() => dispatch(setAccount())}>
              <Icon type="retweet" /> {t('account.switchAccount')}
            </Col>
            <Col style={{ cursor: 'pointer', fontSize: 13 }} onClick={() => this.showCustomTokensForm()}>
              <Icon type="plus" /> {t('account.token.addCustom')}
            </Col>
          </Row>
          <Row type="flex" justify="space-between" align="middle" style={{ paddingTop: 10 }}>
            <Col xs={20}>
              <ChainNetworkSwitch />
            </Col>
            <Col xs={4}>
              <Tooltip placement="top" trigger="hover" title={t('account.refresh')}>
                <Button
                  shape="circle"
                  icon="redo"
                  style={{
  ...
  private readonly renderClaimConfirmation = (): JSX.Element | null => {
    const { account } = this.props;
    const { claimConfirmationVisible, claimTokenAddress, authMessage, gasEstimation } = this.state;
  ...
  private readonly renderBidConfirmation = (): JSX.Element | null => {
    const { account } = this.props;
    const { bidConfirmationVisible: bidConfirmation, bidAmount, gasEstimation } = this.state;
    if (!account || !gasEstimation) {
      return null;
    }
    const dataSource = {
